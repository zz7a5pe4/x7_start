...
class Flags( object ):
    rabbit_max_retries = 0     #maximum rabbit connection attempts (0=try forever)
    rabbit_durable_queues = False #use durable queues
    control_exchange = 'X7'  #the main exchange to connect to

    rabbit_durable_queues = False
    rabbit_retry_interval = 1    #rabbit connection retry interval to start
    rabbit_retry_backoff = 2     #rabbit connection retry backoff in seconds
...
#                    'after %(max_retries)d tries: %(err_str)s') % locals())
            sys.exit(1)
#        LOG.info(_('Connected to AMQP server on %(hostname)s:%(port)d' % self.params))
...
#                ' unreachable: %(e)s. Trying again in %(intv)d seconds.') % info)
